...
def test_hash_verified_lockfiles_exist_for_both_groups() -> None:
    for name in ("requirements.lock.txt", "requirements-dev.lock.txt"):
        path = API / name
        assert path.is_file(), f"missing lockfile: {name}"
        text = path.read_text()
        assert "--hash=sha256:" in text, f"{name} is not hash-verified"
        # Resolved on the runtime interpreter, not a developer's newer Python.
        assert "with Python 3.12" in text, f"{name} was not resolved on the runtime interpreter"


def test_runtime_lock_excludes_development_tooling() -> None:
    """The production image installs the runtime lock; test tooling must not be in it."""
    runtime = (API / "requirements.lock.txt").read_text().lower()
    for tool in ("pytest==", "ruff==", "mypy==", "black=="):
        assert tool not in runtime, f"development tool {tool} leaked into the runtime lock"


def test_dev_lock_includes_test_tooling() -> None:
    dev = (API / "requirements-dev.lock.txt").read_text().lower()
    assert "pytest==" in dev


def test_image_installs_from_the_lock_with_hash_verification() -> None:
    dockerfile = (API / "Dockerfile").read_text()
    assert (
        "--require-hashes -r requirements.lock.txt" in dockerfile
    ), "the image must install from the hash-verified lock"
    # --no-deps ensures the package install cannot silently re-resolve around the lock.
    assert "--no-deps ." in dockerfile


def test_ci_installs_from_the_lock() -> None:
    ci = _ci()
    assert "--require-hashes -r requirements-dev.lock.txt" in ci
    assert "pip install -e '.[dev]'" not in ci, "CI must not resolve dependencies outside the lock"


def test_lock_regeneration_is_scripted_and_pinned() -> None:
    """Locks are generated by a script against the pinned base image, never hand-edited."""
    script = (REPO / "scripts" / "lock_python_dependencies.sh").read_text()
    assert "pip-compile" in script and "--generate-hashes" in script
    dockerfile_digest = (
        [
            line
            for line in (API / "Dockerfile").read_text().splitlines()
            if line.startswith("FROM ")
        ][0]
        .split("@")[1]
        .strip()
    )
    assert dockerfile_digest in script, "lock script must use the same base image as the runtime"
