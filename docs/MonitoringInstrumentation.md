...
The registration plan and health metadata expose active in-process monitor types. Future browser, AI paste, database listener, RAG, MCP, and agent-session adapters should call the same text scan boundary and send accepted raw events to future alerting and incident reconstruction. Matching is O(T × P) for active trace count T and payload length P; it intentionally avoids fuzzy matching.
